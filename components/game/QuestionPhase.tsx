  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: 8, maxWidth: 400, margin: '0 auto' }}>
        ...
        />
        <button className="btn btn-primary" disabled={disabled || !val.trim()} onClick={() => onSubmit(val.trim())}>SUBMIT</button>
      </div>
    </div>
  );
}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center', maxWidth: 500, margin: '0 auto' }}>
        {alphabet.map(l => (
          <button key={l} className="btn btn-outline btn-sm"
            disabled={disabled || hangmanGuessed.includes(l) || hangmanSolved}
            style={{
...
export default function QuestionPhase() {
  ...
  return (
    <div className="page-content" style={{ justifyContent: 'center', gap: 16, minHeight: '70vh' }}>
      ...
      >
        {q.type === 'scramble' && <ScrambleQ q={q} onSubmit={a => submitAnswer(a)} disabled={answered} />}
        {q.type === 'riddle' && <RiddleQ q={q} onSubmit={a => submitAnswer(a)} disabled={answered} />}
        {q.type === 'image_mcq' && <ImageMCQ q={q} onSubmit={a => submitAnswer(a)} disabled={answered} />}
        {q.type === 'hangman' && <HangmanQ disabled={answered} />}
        {q.type === 'mcq' && <MCQ q={q} onSubmit={a => submitAnswer(a)} disabled={answered} />}
      </motion.div>
