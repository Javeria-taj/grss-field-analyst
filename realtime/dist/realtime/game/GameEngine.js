...
class GameEngine {
    ...
    trackPowerup(usn, result) {
        if (!this._playerPowerups)
            this._playerPowerups = new Map();
        const map = this._playerPowerups;
        if (!map.has(usn))
            map.set(usn, {});
    ...
    getStateForClient(usn) {
        ...
        return {
            ...
            anomalyData: this.phase === 'anomaly_active' ? {
                type: 'patch',
                anomalyType: this._lastAnomalyType || 'whack_a_mole',
                targetIds: ['WIN_TOKEN'],
                targetId: 'WIN_TOKEN',
                gridSize: 9,
                timeLimit: 15
            } : null,
            activePowerups: this._playerPowerups?.get(usn) || {},
        };
    }
    getAdminStats() {
        return {
    ...
    reset() {
        ...
        this.recentReactions = [];
        this.factionScores = { team_sentinel: 0, team_landsat: 0, team_modis: 0 };
        this._preAnomalyPhase = null;
        this._savedTimerRemaining = null;
    ...
    // ═══════════════════════════════════════════════════════════════
    triggerAnomaly() {
        if (this.phase === 'game_over' || this.phase === 'anomaly_active')
            return;
        // Save current phase so we can restore it after the anomaly ends
        this._preAnomalyPhase = this.phase;
        // If a question is active, snapshot the remaining time so we can resume it
        if (this.phase === 'question_active') {
            this._savedTimerRemaining = Math.max(5, this.timerRemaining); // min 5s on resume
        }
        else {
            this._savedTimerRemaining = null;
        }
        ...
        let anomalyType = 'whack_a_mole';
        if (this.currentLevel === 2)
            anomalyType = 'overload';
        if (this.currentLevel === 3)
            anomalyType = 'sliders';
        if (this.currentLevel === 4)
            anomalyType = 'wire_routing';
        this._lastAnomalyType = anomalyType;
        ...
        MissionCommander_1.MissionCommander.generateCommentary(`ZERO-DAY DETECTED: ${anomalyType.toUpperCase().replace(/_/g, ' ')}`, 'WIN_TOKEN', { distribution: {}, factionScores: {} }, this.getPlayerCount(), this.phase).then(commentary => {
    ...
    endAnomaly() {
        ...
        const savedPhase = this._preAnomalyPhase;
        const savedTime = this._savedTimerRemaining;
    ...
    async hydrateFromDb() {
        try {
            ...
            this.l5phase = snap.l5phase ?? null;
